...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        last_shape = input_dim
        for i, now_shape in enumerate(hidden_dims):
            self.params['W'+str(i+1)] = weight_scale * np.random.randn(last_shape, now_shape)
            self.params['b'+str(i+1)] = np.zeros(now_shape)
            last_shape = now_shape
            if self.normalization is not None:
                self.params['gamma'+str(i+1)] = np.ones(now_shape)
                self.params['beta'+str(i+1)] = np.zeros(now_shape)
        self.params['W' + str(self.num_layers)] = weight_scale * np.random.randn(last_shape, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.normalization=="batchnorm":
                ...
                layer_input, ar_cache[i-1] = affine_ln_relu_forward(
                    layer_input, self.params['W'+str(i)], self.params['b'+str(i)],
                    self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
            else:
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization=="batchnorm":
                ...
                dout, dw, db, dgamma, dbeta = affine_ln_relu_backward(dout, ar_cache[layer-1])
                grads['gamma' + str(layer)] = dgamma
                grads['beta' + str(layer)] = dbeta
            else:
